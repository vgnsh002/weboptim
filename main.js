...
var resizePizzas = function(size) {
  ...
  //Placed outside and replaced with getElementById
      var SizeOfPizza = document.querySelector("#pizzaSize");
      switch (size) {
          case "1":
              SizeOfPizza.innerHTML = "Small";
              return;
          case "2":
              SizeOfPizza.innerHTML = "Medium";
              return;
          case "3":
            SizeOfPizza.innerHTML = "Large";
              return;
          default:
  ...
  function changePizzaSizes(size) {
    switch (size) {
        case "1":
            newSize = 20; // Whole values in percent for easy computation
            break;
        case "2":
            newSize = 30;
            break;
...
// Redundancy is reduced by placing outside the loop (DRY CODE - DO NOT REPEAT YOUR CODE suugested by A cool reviewer !)
...
function updatePositions() {
  ...
  var scrollVar = (document.body.scrollTop / 1250);//Looping redundancy avoided by placing outside loop
  var j= items.length;

  for (var i = 0; i < j; i++) {
    var calc= Math.sin((scrollVar) + (i % 5)); //Eliminates FSL
    items[i].style.left = items[i].basicLeft + 100 * calc + 'px';
      }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8; //Restored to 8 as per reviewer suggestion
  var s = 256;
  var elem;
  var row = Math.round(window.screen.height/s) // To proportionate the number of Pizzas
  var pizzaNum = row*cols//This line makes the pizza vary accordingly with respect to the size of Screen
